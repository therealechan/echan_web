...
set :repository, "git@bitbucket.org:therealechan/echan-web.git"
...
namespace :deploy do
  ...
  task :symlink_shared, :roles => :app do
    %w{database private_pub settings.local}.each do |file|
...
namespace :update do
  ...
  task :database do
    ...
    remote_sql_file_path        = "#{current_path}/tmp/#{rails_env}-#{remote_settings["database"]}-dump.dump"
    remote_sql_gz_file_path     = "#{remote_sql_file_path}.gz"
    local_sql_file_path         = "tmp/#{rails_env}-#{remote_settings["database"]}-#{Time.now.strftime("%Y-%m-%d_%H:%M:%S")}.dump"
    ...
    run "PGPASSWORD='#{remote_settings['password']}' pg_dump  -Fc -U #{remote_settings["username"]} #{"-h '#{remote_settings["host"]}'" if remote_settings["host"]} -c -O '#{remote_settings["database"]}' > #{remote_sql_file_path}"
    ...
    run_locally("PGPASSWORD='#{local_settings['password']}' pg_restore --verbose --clean --no-acl --no-owner -h localhost -U #{local_settings["username"]} -d #{local_settings["database"]} #{local_sql_file_path}")
  ...
  namespace :remote do
    ...
    task :shared_assets do
      # STDOUT.puts "Are you SURE to update the databse of remote?(YES)"
      # confirmation = STDIN.gets.chomp
      confirmation = Capistrano::CLI.ui.ask("Are you SURE to upload the local assets to remote?(YES)")
      abort "Interrupt.." unless confirmation == "YES"
